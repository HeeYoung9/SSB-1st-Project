		...
		<a class="sidebar-toggle js-sidebar-toggle">
          <i class="hamburger align-self-center"></i>
			...
			<main id="item" style="width: 1500px;" align="center">
				<section class="list">
					<span> <a href="./ItemMgt.it"> <img class="Mgt"
							src="./main/item_img/itemMgt.png" width="60" height="60">
					</a>
						<h1>상품 관리</h1>
						<form id="searchForm">
						...
						<button id="addButton" onclick="location.href='./itemAddForm.it';">상품
							등록</button> <!-- <button id="editButton">상품 수정</button> -->
						<form action="./ItemMgt.it?item_style=${itemStyle }" method="get"
							class="search-bar" style="display: inline-block;">
							<select id="sel" name="item_style" class="form-select"
								aria-label="Default select example"
								style="width: 100px; float: left;">
								<option value="sale"
									${param.item_style=='sale' ? 'selected="selected"' : ''}>판매</option>
								<option value="rental"
									${param.item_style=='rental' ?'selected="selected"' : '' }>렌탈</option>
							</select> <input type="submit" value="검색" id="searchButton "></input>
						</form>
					</span>
					...
					<table class="sort">
						...
						<c:if test="${itemStyle=='rental' }">
							<c:forEach var="rdto" items="${rItemMgt }">
								<tr style="background-color: white;">
									<td><label class="checkbox-inline"> <input
											type="checkbox" name="options_id"
											value="${rdto.rental_item_id }">
									</label></td>
					...
					<c:if test="${itemStyle==null || itemStyle=='sale' }">
						<div class="paging">
							...
							<span class="num"> <c:forEach var="i"
									...
									<a
										href="./ItemMgt.it?pageNum=${i }&search=${param.search}&item_style=${param.item_style}&search=${param.search}"
										class="on">${i }</a>
								</c:forEach>
							</span>
					...
					<c:if test="${itemStyle=='rental' }">
						<div class="paging">
							...
							<span class="num"> <c:forEach var="i"
									begin="${startPage }" end="${endPage }" step="1">
									<a
										href="./ItemMgt.it?pageNum=${i }&search=${param.search}&item_style=${param.item_style }"
										class="on">${i }</a>
								</c:forEach>
							</span>
