                ...
                --%>
					...
					<div class="paging">
						<c:if test="${startPage > pageBlock }">
							<a
								href="./AdOrderList.od?pageNum=${startPage-pageBlock }&state=${param.state}">이전</a>
						</c:if>

						<c:forEach var="i" begin="${startPage }" end="${endPage }"
							step="1">
							<span class="num"> <a
								href="./AdOrderList.od?pageNum=${i }&state=${param.state}"
								class="on">${i }</a>
							</span>
						</c:forEach>
						...
						<c:if test="${endPage < pageCount }">
							<a
								href="./AdOrderList.od?pageNum=${startPage + pageBlock}&state=${param.state}">다음</a>
						</c:if>
					</div>
				</section>
			</main>
