...
public class BoardDAO {
	...
	// 공지사항 글쓰기 메서드 - insertNoticeBoard(BoardDTO)
	public void insertNoticeBoard(BoardDTO bdto) {
		...
		try {
			...
			// 5. 데이터 처리
			if(rs.next()) {
				// board_id = rs.getInt("max(board_id)") + 1;
			...
			System.out.println("DAO: " + id + "번 글쓰기 완료!");		
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			CloseDB();
	...
	public int getBoardCount() {
		...
		try {
			...
			sql = "select count(*) from board_remaster where board_type=?";
			pstmt = con.prepareStatement(sql);			
	...
	public ArrayList getNoticeList(int startRow, int pageSize) {
		...
		try {
			...
			sql = "select * from board_remaster order by board_id desc limit ?,?";
			pstmt = con.prepareStatement(sql);
